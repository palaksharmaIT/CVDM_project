...
def content_list(request):
    """
    Full list of content with quick status filters. This is the
    entry point for creating new drafts and opening the editor.
    """

    status_filter = request.GET.get("status", "")

    contents = Content.objects.select_related("created_by").all()
    ...
    context = {
        ...
        "can_create": _user_has_role(request.user, "Author", "Admin"),
    }

    return render(request, "dashboard/content_list.html", context)
...
def content_edit(request, content_id):
    ...
    can_edit = is_author and content.status in (
