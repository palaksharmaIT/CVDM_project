...
def content_edit(request, content_id):
    ...
    if can_assign:
        ...
        recommended_reviewers = get_recommended_reviewers(
            content=content,
            eligible_reviewers=eligible_reviewers,
        )
        ...
        if error:
            ...
            try:
                with transaction.atomic():
                    ...
                    # Raises on failure — inside the atomic block so a
                    # failed send rolls back the user/group/verification
                    # rows instead of leaving an orphaned, permanently
                    # inactive account behind.
                    send_verification_email(username, email, verify_url)
            except Exception:
                messages.error(
                    request,
